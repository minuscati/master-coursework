...
def MiniBatchGD(
    ...
    MX_te=None, y_te=None, record_test_every=5, test_eval_size=2000
):
    n = MX_tr.shape[2]
...
trained_net, metrices = MiniBatchGD(
    MX_tr, Y_tr, y_tr, GDparams, net_params, lam, MX_val, y_val,
    MX_te=MX_te, y_te=y_te, record_test_every=1, test_eval_size=10000
)
...
loss_te, cost_te, acc_te = metrices['loss_te'], metrices['cost_te'], metrices['acc_te']
...
acc_te = np.array(metrices['acc_te'], dtype=np.float64)
loss_te = np.array(metrices['loss_te'], dtype=np.float64)

# Full-test metrics at the end of training (for final report value)
P_te_full, _ = forward_pass(
    MX_te,
    trained_net['Fs_flat'],
    trained_net['W1'],
    trained_net['b1'],
...
acc_te_full = ComputeAccuracy(P_te_full, y_te)
loss_te_full = ComputeLoss(P_te_full, y_te)

#%%
# plot loss
plt.figure(figsize=(10, 6))
plt.plot(step_ax, loss_tr, color='blue', label='training loss')
plt.plot(step_ax, loss_te, color='orange', label='test loss')
plt.xlabel('update step')
plt.ylabel('loss')
plt.ylim(0,2.5)
# plt.xlim(left=0)
...
# plot accuracy
plt.figure(figsize=(10, 6))
plt.plot(step_ax, acc_tr, color='blue', label='training accuracy')
plt.plot(step_ax, acc_te, color='orange', label='test accuracy')
plt.xlabel('update step')
plt.ylabel('accuracy')
plt.ylim(0,1)
# plt.xlim(left=0)
...
print(f"Final full-test accuracy: {acc_te_full:.6f}")
