...
def augment_cifar_batch(
    ...
    """
    Apply random crop + horizontal flip to a CIFAR batch.
    X_batch shape: (3072, n)
    Returns augmented batch with same shape and dtype.
    """
    d, n = X_batch.shape
    if d != 32 * 32 * 3:
...
for arch in architectures:
    ...
    trained_arch, metrices = MiniBatchGD(
        ...
        X_tr_raw=X_tr, f_conv=f_arch, use_data_aug=use_data_aug, aug_pad=aug_pad, aug_flip_prob=aug_flip_prob,
        dropout_p=dropout_p
    )
...
#%%
    ...
    print(f"{arch['name']}: final test acc={acc_arch:.6f}, train time={train_elapsed:.2f}s")
